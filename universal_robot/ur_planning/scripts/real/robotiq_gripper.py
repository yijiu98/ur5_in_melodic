...
class RobotiqGripper:
    ...
    #def connect(self, hostname: unicode, port: int, socket_timeout: float = 2.0) -> None:
    def connect(self,  hostname,  port,  socket_timeout=2.0, encoding="utf-8"): 
    ...
    def _set_vars(self, var_dict):
        ...
        for variable, value in var_dict.items():
            ...
            cmd += ' {0} {1}'.format(variable,str(value))
            #cmd += '{variable:format} {str(value):format}'
    ...
    def _get_var(self, variable):
        ...
        if var_name != variable:
            ...
            raise ValueError("Unexpected response {0} ({data.decode(1)}): does not match '{2}'".format(data,self.ENCODING,variable))
        value = int(value_str)
        return value
    ...
    def auto_calibrate(self, log=True):
        ...
        if log:
            print("Gripper auto-calibrated to [{self.get_min_position()}, {self.get_max_position()}]")
